...
exports.executeExportJob = executeExportJob;
...
async function executeExportJob(plan, settings, cache, mainWindow, trackProcessFn) {
    ...
    for (let idx = 0; idx < plan.mainTrack.length; idx++) {
        ...
        if (isImage) {
            ...
            await convertImageToVideoSegment(clip.srcPath, segPath, durationSec, useSourceResolution ? null : targetWidth, useSourceResolution ? null : targetHeight, clip.assetWidth, // Pass asset dimensions for aspect ratio preservation
            clip.assetHeight, settings.bitrate, trackProcessFn);
        ...
        () => {
            return new Promise((resolve, reject) => {
                ...
                const ffmpegProcess = command
                    .output(outputPath)
                    .on('end', () => {
                    if (trackProcessFn && ffmpegProcess)
                        trackProcessFn(ffmpegProcess);
                    resolve();
                })
                    .on('error', (err) => {
                    if (trackProcessFn && ffmpegProcess)
...
function trimSegment(inputPath, outputPath, startSec, durationSec, copyCodec, targetWidth, targetHeight, bitrate, trackProcessFn) {
    return new Promise((resolve, reject) => {
        const command = (0, fluent_ffmpeg_1.default)(inputPath).seekInput(startSec).duration(durationSec);
        if (copyCodec) {
